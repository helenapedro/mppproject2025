...
public class JdbcClientRepo implements ClientRepo {
    ...
    @Override public boolean deleteById(Integer id){ try(Connection c=DB.getConnection(); PreparedStatement ps=c.prepareStatement("DELETE FROM client WHERE id=?")){ ps.setInt(1,id); return ps.executeUpdate()>0; } catch(SQLException e){ throw new RuntimeException(e);} }
}
